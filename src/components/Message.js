...
function Message() {
    ...
    const addMessage = async() => {
        ...
        const messageRef = collection(messageDoc,`Message-${location.state.display}`)
        try{
            await addDoc(messageRef,{
                message:message,
                username:location.state.currentUserName,
                profile_image:location.state.currentUserProImg,
            })
        }catch(err){
            console.error(err)
        }
        ...
        <div>
            {messageData.map((userMessage)=>{
                return<>
                    ...
                    <h5 style={{marginLeft:"30px", fontWeight:"100"}}>
                        {userMessage.message}
                    </h5>
                </>
            })}
